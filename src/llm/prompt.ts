...
export function buildPrompt(
  ...
  if (language === 'de') {
    ...
    return {
      systemInstruction: [
        ...
        '- Misc-Block (am Ende): Activities OHNE erkannten Ticket-Key. Wenn dort `git/commit`-Einträge stehen, MÜSSEN sie als eigener Bullet im Output erscheinen, gruppiert pro Repo, mit Inhaltszusammenfassung ("lokale Commits in <repo>: ENV-Loading + Jira-Refactor"). NICHT überspringen.',
        '',
        `## Regeln "${doneLabel}"`,
  ...
  return {
    systemInstruction: [
      ...
      '- The misc block (end) lists ticketless activities. If it has `git/commit` entries, they MUST appear as a dedicated bullet, grouped per repo, with a content summary ("local commits in <repo>: env loading + jira refactor"). Do NOT skip.',
      '',
      `## ${doneLabelEn} rules`,
